      ...
      :data="lists">
        <el-table-column label="头像" prop="avatarUrl">
          <template slot-scope='scope'>
            <img v-if="scope.row.avatarUrl" :src="scope.row.avatarUrl" alt="" class="avatarUrl">
            <!-- <span v-if="scope.row.nickName" v-text="scope.row.nickName"></span> -->
...
export default {
  ...
  methods: {
    ...
    getViewsLists(skip = this.skip, limit = this.limit) {
      ...
      url
        .post(`/loginLists`, { condition, limit: limit, skip: skip * limit })
        .then(res => {
